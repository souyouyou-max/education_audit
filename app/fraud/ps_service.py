...
3. 照片/人像区（重点）：
   - 照片边缘是否有切割/融合/羽化/硬边痕迹，与背景分辨率/噪声是否一致
   - 照片光照方向是否与证件整体环境匹配
   - ★ 钢印/骑缝章痕迹：真实证件的钢印压痕通常跨越照片边缘，在照片上留下浮雕形变；若照片区域完全没有钢印压痕、而证件其他区域存在钢印，则强烈提示照片为替换/P上去的
   - ★ 照片替换特征：照片与证件底纸的JPEG压缩层级差异、噪声纹理不连续、照片区域过于"新鲜干净"均为替换信号
