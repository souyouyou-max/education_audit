...
async def upload_images(files: List[UploadFile] = File(..., description="一个或多个学历图片")):
    ...
    - 多文件并发处理：所有图片同时提取向量（ML 推理在线程池中执行）
    ...
    loop = asyncio.get_running_loop()  # Python 3.10+ 兼容写法，get_event_loop() 在 async 函数内已废弃

    async def _process_one(filename: str, file_content: bytes) -> BatchUploadItem:
        """单张图片的完整处理流程，ML 推理及 Milvus/磁盘 IO 均在线程池执行"""
        try:
            if not validate_image_file(file_content, filename):
                return BatchUploadItem(
                    filename=filename, success=False,
                    error="无效图片文件，支持 JPG/JPEG/PNG/BMP，最大 10MB",
                )
            image = load_image_from_bytes(file_content)
            logger.info("Extracting vectors: %s", filename)

            # ML 推理（CLIP + InsightFace + DINOv2）放到线程池，不阻塞事件循环
            image_vector, face_vector, template_vector = await loop.run_in_executor(
                None, vector_service.extract_all_vectors, image
            )
            has_face = face_vector is not None and not all(v == 0.0 for v in face_vector)

            # Milvus 插入 + 文件写入 + MySQL 写入同样是阻塞 IO，也放到线程池
            def _do_persist():
                iid = milvus_client.insert(
                    image_vector=image_vector,
                    face_vector=face_vector,
                    template_vector=template_vector,
                )
                try:
                    save_image_by_id(file_content, iid, filename)
                    save_certificate(iid, filename, has_face)
                except Exception:
                    milvus_client.delete_by_id(iid)  # 文件写入失败时回滚，避免孤儿记录
                    raise
                return iid

            inserted_id = await loop.run_in_executor(None, _do_persist)
            logger.info("Uploaded: %s -> id=%s", filename, inserted_id)
            ...
            return BatchUploadItem(filename=filename, success=False, error=str(e))

    # 所有图片并发处理
    items: List[BatchUploadItem] = list(
        await asyncio.gather(*[_process_one(fn, fc) for fn, fc in file_data])
    )

    success_count = sum(1 for i in items if i.success)
    fail_count    = len(items) - success_count
    message = f"成功 {success_count} 张，失败 {fail_count} 张" if len(items) > 1 else (
        "上传成功" if success_count == 1 else "上传失败"
    )
    return BatchUploadResponse(items=items, success_count=success_count,
                               fail_count=fail_count, message=message)
...
async def search_similar(request: SearchRequest):
    ...
    if request.id is not None:
        # request.id 现在是字符串（防 JS 精度丢失），内部转 int 供 Milvus 使用
        try:
            query_id_int = int(request.id)
        except (ValueError, TypeError):
            raise HTTPException(status_code=400, detail=f"Invalid ID format: {request.id}")
        query_id = request.id  # 保留字符串形式用于响应
        # get_by_id 是阻塞 gRPC 调用，放入线程池避免阻塞事件循环
        data = await loop.run_in_executor(None, milvus_client.get_by_id, query_id_int)
        if not data:
    ...
    # milvus search 是阻塞 gRPC 调用，放入线程池避免阻塞事件循环
    results = await loop.run_in_executor(
        None,
        lambda: milvus_client.search(
    ...
    formatted_results = [
        SearchResultItem(
            id=str(r["id"]),   # 转字符串，防止 JS 大整数精度丢失
            score=r["score"],
            distance=r["distance"],
...
async def search_by_image(
    ...
    # ML 推理（CLIP + InsightFace + DINOv2）是 CPU 密集型，放入线程池
    image_vector, face_vector, template_vector = await loop.run_in_executor(
        None, vector_service.extract_all_vectors, image
    )
    ...
    # milvus search 是阻塞 gRPC 调用，放入线程池
    results = await loop.run_in_executor(
        None,
...
async def cleanup_orphaned_records():
    """
    清理孤儿记录：删除 Milvus 中没有对应图片文件的向量

    - 扫描 image 集合中全部 entity_id
    - 对每个 ID 检查磁盘上是否存在对应图片文件
    - 删除无文件对应的记录（三个集合同步删除）
    - 返回删除数量和剩余数量
    """
    entity_ids = milvus_client.get_all_entity_ids()
    orphaned = [eid for eid in entity_ids if not get_image_path_by_id(eid)]
    for eid in orphaned:
        milvus_client.delete_by_id(eid)
    logger.info("Cleanup: deleted %d orphaned records, %d remaining",
                len(orphaned), len(entity_ids) - len(orphaned))
    return {
        "deleted": len(orphaned),
        "remaining": len(entity_ids) - len(orphaned),
    }
